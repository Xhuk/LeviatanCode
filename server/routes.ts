...
`;
      ...
      if (analysisResult.pythonScript) {
        ...
        try {
          ...
          const scriptPath = require('path').join(workingDirectory || process.cwd(), scriptFileName);
          
          require('fs').writeFileSync(scriptPath, analysisResult.pythonScript, 'utf8');
...
`;
          ...
          const readmePath = require('path').join(workingDirectory || process.cwd(), `${projectId}_analysis_README.md`);
          require('fs').writeFileSync(readmePath, readmeContent, 'utf8');
